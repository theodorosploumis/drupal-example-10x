...
class ComponentValidator {
  ...
  public function validateDefinition(array $definition, bool $enforce_schemas): bool {
    ...
    if (!$schemas) {
      if ($enforce_schemas) {
        throw new InvalidComponentException(sprintf('The component "%s" does not provide schema information. Schema definitions are mandatory for components declared in modules. For components declared in themes, schema definitions are only mandatory if the "enforce_sdc_schemas" key is set to "true" in the theme info file.', $definition['id']));
      }
      return TRUE;
    }
    $classes_per_prop = $this->getClassProps($schemas['props'] ?? []);
    ...
    $definition['schemas']['props'] = $this->nullifyClassPropsSchema(
      $schemas['props'],
      $classes_per_prop
    );
  ...
  public function validateProps(array $context, Component $component): bool {
    ...
    $schemas = $component->getMetadata()->getSchemas();
    if (!$schemas) {
      if ($component->getMetadata()->shouldEnforceSchemas()) {
        throw new InvalidComponentException(sprintf('The component "%s" does not provide schema information. Schema definitions are mandatory for components declared in modules. For components declared in themes, schema definitions are only mandatory if the "enforce_sdc_schemas" key is set to "true" in the theme info file.', $component_id));
      }
      return TRUE;
    }
    $props_schema = $schemas['props'] ?? [];
    $prop_names = array_keys($props_schema['properties'] ?? []);
    ...
    [
      $props_schema,
      $props_raw,
    ] = $this->validateClassProps($props_schema, $props_raw, $component_id);
    $props_schema = Validator::arrayToObjectRecursive($props_schema);
    $props = Validator::arrayToObjectRecursive($props_raw);
    $throw_error = static function (array $errors) {
      $message_parts = array_map(
        static fn(array $error): string => sprintf("[%s] %s", $error['property'], $error['message']),
        $errors
      );
    ...
    $validator->validate($props, $props_schema);
    $validator->getErrors();
    ...
    $errors = array_filter(
      ...
      function (array $error) use ($context): bool {
        ...
        return !$this->isRenderArray($context[$error['property']] ?? NULL);
      }
    );
    if (empty($errors)) {
      return TRUE;
   ...
   */
  protected function isRenderArray($candidate): bool {
    if (!is_array($candidate)) {
      return FALSE;
    }
    $children_keys = Element::children($candidate);
    if (empty($children_keys)) {
      // This is a render array, if there is at least one non-children element.
      return !empty($candidate);
    }
    foreach ($children_keys as $key) {
      if (!$this->isRenderArray($candidate[$key] ?? NULL)) {
        return FALSE;
      }
    }
    return TRUE;

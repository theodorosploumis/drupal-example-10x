...
/**
 * Checks weather or not two schemas are compatible.
 *
...
class SchemaCompatibilityChecker {
  ...
  /**
   * Checks if the replacement schema is fully compatible with the old one.
   *
   * The goal is to ensure existing usages of the original component will not
   * break when the new component takes place. It will also ensure that if we
   * remove the new schema in the future and we fall back to the original
   * component, the usages will not break.
   *
   ...
   */
  public function isFullyCompatible(array $original_schema, array $new_schema): void {
    $this->isCompatible($original_schema, $new_schema);
    $this->isCompatible($new_schema, $original_schema);
  }
